...
async def call_agent(agent_url: str, message: str, timeout_sec: Optional[float] = None) -> Any:
    """
    Convenience function to call an agent with a text message.
    ...
    async with A2AClient(agent_url) as client:
        return await client.send_message(message, timeout_sec)